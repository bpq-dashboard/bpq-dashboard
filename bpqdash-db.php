 ...
 *   $pdo = bpqdash_db();
 *   $rows = bpqdash_query($pdo, "SELECT * FROM sessions WHERE hub = ?", ['YOURCALL-7']);
 *
 * The connection is a singleton — multiple require_once calls share one PDO.
 */

// ── Credentials ───────────────────────────────────────────────────────────────
define('BPQDASH_DB_HOST', 'localhost');
define('BPQDASH_DB_NAME', 'bpqdash');
define('BPQDASH_DB_USER', 'bpqdash');
define('BPQDASH_DB_PASS', 'CHANGE_THIS_DB_PASSWORD');
define('BPQDASH_DB_PORT', 3306);

/**
 * Return the singleton PDO connection.
 * Throws PDOException on failure (caught at call site or bubbles up as 500).
 */
function bpqdash_db(): PDO {
    static $pdo = null;
    if ($pdo === null) {
        $dsn = sprintf(
            'mysql:host=%s;port=%d;dbname=%s;charset=utf8mb4',
            BPQDASH_DB_HOST, BPQDASH_DB_PORT, BPQDASH_DB_NAME
        );
        $pdo = new PDO($dsn, BPQDASH_DB_USER, BPQDASH_DB_PASS, [
 ...
 */
function bpqdash_query(PDO $pdo, string $sql, array $params = []): array {
 ...
 */
function bpqdash_query_one(PDO $pdo, string $sql, array $params = []): ?array {
    $stmt = $pdo->prepare($sql);
    $stmt->execute($params);
    $row = $stmt->fetch();
 ...
 */
function bpqdash_execute(PDO $pdo, string $sql, array $params = []): int {
    $stmt = $pdo->prepare($sql);
    $stmt->execute($params);
 ...
 */
function bpqdash_insert_session(PDO $pdo, array $s): int {
    $sql = "
    ...
    try {
        ...
        error_log('bpqdash_insert_session error: ' . $e->getMessage());
        return 0;
    }
}
 ...
 */
function bpqdash_upsert_station(PDO $pdo, string $callsign, string $date,
    ...
    try {
        ...
        error_log('bpqdash_upsert_station error: ' . $e->getMessage());
    }
}
 ...
 */
function bpqdash_db_available(): bool {
    try {
        $pdo = bpqdash_db();
        $pdo->query('SELECT 1');
        return true;
 ...
 */
function bpqdash_duration_to_secs(?string $duration): ?int {
