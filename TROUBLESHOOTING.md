...
```bash
sudo nano /var/www/bpq-dashboard/config.php
```
...
```bash
cat /var/www/bpq-dashboard/cache/aprs/aprs-daemon.json
```
...
```bash
sudo rm -f /var/www/bpq-dashboard/cache/aprs/stations.json
sudo systemctl restart bpq-aprs
```
...
```bash
sudo chown -R www-data:www-data /var/www/bpq-dashboard
sudo chmod -R 755 /var/www/bpq-dashboard
sudo chmod -R 775 /var/www/bpq-dashboard/cache /var/www/bpq-dashboard/data
sudo chmod 640 /var/www/bpq-dashboard/config.php
```
...
**Test connection:**
```bash
mysql -u bpqdash -p bpqdash -e "SELECT 1"
```

**Reset database password:**
```bash
sudo mysql
ALTER USER 'bpqdash'@'localhost' IDENTIFIED BY 'your-new-password';
FLUSH PRIVILEGES;
exit
```
