...
if (file_exists(__DIR__ . '/bpqdash-db.php')) {
    ...
    $DB_ENABLED = bpqdash_db_available();
}
...
function getParsedSyslog($syslogPath, $hubIds, $cacheDir, $cacheTTL) {
    ...
    global $DB_ENABLED;
    if ($DB_ENABLED) {
        bpqdash_db_write_sessions($parsed, gmdate('Y-m-d'), array_map('strtoupper', $hubIds));
    }
 ...
 */
function bpqdash_db_write_sessions(array $parsed, string $todayDate, array $hubIds): void {
    try {
        $pdo  = bpqdash_db();
        $year = (int)date('Y');
        ...
        foreach ($parsed['stationSessions'] ?? [] as $key => $sessions) {
            ...
            foreach ($sessions as $sess) {
                ...
                bpqdash_upsert_station($pdo, $station, $todayDate);
            }
        }
    } catch (Throwable $e) {
