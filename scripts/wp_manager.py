...
CRON EXAMPLE (run daily at 03:00 UTC):
  0 3 * * * cd /home/tony/linbpq && python3 /var/www/bpq-dashboard/scripts/wp_manager.py --auto-clean >> /var/log/wp-auto-clean.log 2>&1
...
def cmd_apply(wp_path):
    ...
    # Restart BPQ so changes take effect (skip if --no-restart flag set)
    import subprocess
    no_restart = '--no-restart' in sys.argv
    if no_restart:
        log("BPQ_RESTART  SKIPPED (--no-restart flag)")
        print(f"  BPQ restart skipped (--no-restart flag)")
        print(f"  Run manually when ready: sudo systemctl restart bpq")
    else:
...
def cmd_auto_clean(wp_path, stale_days, dry_run=False):
    ...
    # Restart BPQ only if --no-restart flag not set
    import subprocess
    no_restart = '--no-restart' in sys.argv
    if no_restart:
        log("AUTO-CLEAN  BPQ_RESTART  SKIPPED (--no-restart flag)")
        print(f"\n  BPQ restart skipped (--no-restart flag set)")
        print(f"  WP.cfg changes will take effect on next BPQ connect cycle")
    else:
        print(f"\n  Restarting BPQ...")
