...
TELNET_PORT="8010"
TELNET_USER="SYSOPUSER"
TELNET_PASS="CHANGE_THIS_PASSWORD"
OUTPUT_FILE="/var/www/bpq-dashboard/logs/nodesrtt.txt"
...
expect {
    -re {[Cc]allsign:} { send "SYSOPUSER\r" }
    -re {[Ll]ogin:} { send "SYSOPUSER\r" }
    -re {[Uu]sername:} { send "SYSOPUSER\r" }
    timeout { puts "Timeout waiting for login prompt"; exit 1 }
}

# Wait for password prompt and send password
expect {
    -re {[Pp]assword:} { send "CHANGE_THIS_PASSWORD\r" }
