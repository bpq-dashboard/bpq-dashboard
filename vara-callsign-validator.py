...
Architecture:
    ...
    VARA HF on Windows  (192.168.1.100 :9025/:9026)
            ...
            while True:
                ...
                if line.upper().startswith("CONNECTED "):
                    ...
                    if len(parts) >= 2:
                        ...
                        if ok:
                            log.info(f"CMD  ACCEPTED  {raw_call!r:15}  (validated as {result})")
                            bpq_sock.sendall(full_line)
                        else:
                            log.warning(f"CMD  REJECTED  {raw_call!r:15}  reason: {result}")
                            try:
