#!/bin/bash
# ============================================================================
# BPQ Dashboard + BPQDASH Network Monitor — Set Permissions & Install Nginx Config
# Run as root: sudo bash set-permissions.sh
# ============================================================================

DASH_DIR="/var/www/bpq-dashboard"
WEB_USER="www-data"
WEB_GROUP="www-data"
...
if [ -f "$DASH_DIR/nginx-bpq-dashboard.conf" ]; then
    echo ""
    ...
    if [ -f "$NGINX_CONF" ]; then
        ...
        if grep -q "X-Content-Type-Options" "$NGINX_CONF" 2>/dev/null; then
            ...
            cp "$DASH_DIR/nginx-bpq-dashboard.conf" "$NGINX_CONF"
    ...
    echo ""
    echo "[1/8] Nginx config — nginx-bpq-dashboard.conf not found in $DASH_DIR, skipping"
fi
...
if command -v curl &>/dev/null && [ -f "$NGINX_CONF" ]; then
    HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 3 "https://dashboard.example.com/config.php" 2>/dev/null)
...
echo "  curl -s -o /dev/null -w '%{http_code}' https://dashboard.example.com/config.php"
echo "  curl -s -o /dev/null -w '%{http_code}' https://dashboard.example.com/includes/"
