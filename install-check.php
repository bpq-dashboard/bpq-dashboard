...
if ($nginx) {
    ...
    $hasCfg = fileOk('/etc/nginx/sites-enabled/bpq-dashboard.conf')
           || fileOk('/etc/nginx/sites-enabled/bpq-dashboard.conf');
    chk('Web Server','Nginx site config active', $hasCfg,
        $hasCfg ? 'bpq-dashboard.conf or bpq-dashboard.conf found' : 'No site config found',
...
$phpFiles = [
    ...
    'bpqdash-config.php'     => 'Config compatibility shim',
    'network-api.php'      => 'Network data API',
    'bpqdash-db.php'         => 'Database layer',
    ...
    'hub-report.php' => 'Hub health report',
];
...
foreach ($service_files as $sf => $label) {
    if (!file_exists($sf)) {
        ...
        $scriptMap = [
            'bpq-chat.service'      => '/var/www/bpq-dashboard/scripts/bpq-chat-daemon.py',
            'bpq-aprs.service'      => '/var/www/bpq-dashboard/scripts/bpq-aprs-daemon.py',
        ];
        $bn = basename($sf);
...
".'*/5 * * * * /usr/bin/python3 /var/www/bpq-dashboard/scripts/connect-watchdog.py >> /var/log/connect-watchdog.log 2>&1');
...
if ($wpSched) chkPass('Cron Jobs','wp_manager --auto-clean (3am daily)', 'Scheduled');
    ...
    '# sudo crontab -e — add:'."
".'0 3 * * * cd /home/tony/linbpq && /usr/bin/python3 /var/www/bpq-dashboard/scripts/wp_manager.py --auto-clean >> /var/log/wp-auto-clean.log 2>&1');
