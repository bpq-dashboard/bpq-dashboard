...
$phpFiles = [
    ...
    'bpqdash-hub-report.php' => 'Hub health report',
];
...
foreach ($service_files as $sf => $label) {
    if (!file_exists($sf)) {
        ...
        $scriptMap = [
            'bpq-chat.service'      => '/var/www/bpqdash/scripts/bpq-chat-daemon.py',
            'bpq-aprs.service'      => '/var/www/bpqdash/scripts/bpq-aprs-daemon.py',
        ];
        $bn = basename($sf);
...
".'*/5 * * * * /usr/bin/python3 /var/www/bpqdash/scripts/connect-watchdog.py >> /var/log/connect-watchdog.log 2>&1');
...
if ($wpSched) chkOk('Cron Jobs','wp_manager --auto-clean (3am daily)', 'Scheduled');
    ...
    '# sudo crontab -e — add:'."
".'0 3 * * * cd /home/tony/linbpq && /usr/bin/python3 /var/www/bpqdash/scripts/wp_manager.py --auto-clean >> /var/log/wp-auto-clean.log 2>&1');
