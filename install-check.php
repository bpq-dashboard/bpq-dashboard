<?php
/**
 * BPQ Dashboard v1.5.6 — Post-Installation Check
...
if ($nginx) {
    ...
    $hasCfg = fileOk('/etc/nginx/sites-enabled/bpq-dashboard.conf')
           || fileOk('/etc/nginx/sites-enabled/bpqdash.conf');
    chk('Web Server','Nginx site config active', $hasCfg,
        $hasCfg ? 'bpq-dashboard.conf or bpqdash.conf found' : 'No site config found',
    ...
    if ($hasCfg) {
        $cfgFile = fileOk('/etc/nginx/sites-enabled/bpq-dashboard.conf')
                 ...
                 : '/etc/nginx/sites-enabled/bpqdash.conf';
...
$phpFiles = [
    ...
    'message-storage.php'  => 'BBS message server storage',
    'geomag-proxy.php'     => 'Geomagnetic data proxy',
    'settings-api.php'     => 'Settings API',
    'health-check.php'     => 'Health check API',
];
...
$cfgPaths = ['/home/linbpq/bpq32.cfg','/home/SYSOP/linbpq/bpq32.cfg',
...
foreach ($service_files as $sf => $label) {
    if (!file_exists($sf)) {
        ...
        chkPass('Services', "$label service file", $sf);
        // Check correct script path
        $scriptMap = [
            'bpq-chat.service'      => '/var/www/html/bpq/scripts/bpq-chat-daemon.py',
            'bpq-aprs.service'      => '/var/www/html/bpq/scripts/bpq-aprs-daemon.py',
        ];
        $bn = basename($sf);
        if (isset($scriptMap[$bn])) {
            ...
            if (strpos($svc, $sp) === false)
                ...
                chkPass('Services', "$label ExecStart path", $sp);
        }
        // www-data user
        if ($bn !== 'vara-validator.service') {
            ...
            if (!$hasUser)
                ...
                chkPass('Services', "$label User=www-data", 'Set correctly');
        }
        // Restart=always
        if (strpos($svc,'Restart=always') === false)
            ...
            chkPass('Services', "$label Restart=always", 'Set');
    }
}
...
if ($wdRoot && !$wdUser)
    chkPass('Cron Jobs','connect-watchdog (root cron, every 5 min)', 'Root only — correct');
...
if ($wpSched) chkPass('Cron Jobs','wp_manager --auto-clean (3am daily)', 'Scheduled');
else chkWarn('Cron Jobs','wp_manager --auto-clean not scheduled',
    'WP.cfg will not be automatically cleaned',
    '# sudo crontab -e — add:'."
".'0 3 * * * cd /home/SYSOP/linbpq && /usr/bin/python3 /var/www/html/bpq/scripts/wp_manager.py --auto-clean >> /var/log/wp-auto-clean.log 2>&1');

// prop-scheduler
if (strpos($allCron,'prop-scheduler') !== false)
    chkPass('Cron Jobs','prop-scheduler scheduled', 'Present in crontab');
else
    chkWarn('Cron Jobs','prop-scheduler not scheduled', 'May be run manually — OK if intended', '');

// storm-monitor
if (strpos($allCron,'storm-monitor') !== false)
    chkPass('Cron Jobs','storm-monitor scheduled', 'Present in crontab');
else
...
if ($malicious)
    ...
    chkPass('Cron Jobs','No malicious cron entries', 'Clean');
...
foreach ([
    ...
    if (!file_exists($lf))
        ...
        chkPass('Log Files',"$label log", "${kb} KB, last modified {$age}m ago");
    }
}
...
<div class="box">
<h2>🔧 BPQ Dashboard</h2>
<p>Post-Installation Check v1.5.6</p>
<form method="get">
...
<title>BPQ Dashboard v1.5.6 — Install Check</title>
<style>
...
<h1>🔧 BPQ Dashboard v1.5.6 — Post-Installation Check</h1>
...
<div class="footer">
    ...
    BPQ Dashboard v1.5.6 · Access: <?=htmlspecialchars("https://{$_SERVER['HTTP_HOST']}/install-check.php?pass=".CHECK_PASS)?>
</div>
</body></html>
<?php }
